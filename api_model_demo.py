...
def shutdown_app(delay_seconds=0.5):
...
def handle_chat_input(client, collection):
    if prompt := st.chat_input("请输入：(输入\\quit退出)"):
        if prompt.strip().lower() == r"\quit":
            shutdown_message = "正在关闭程序..."
            st.session_state.messages.append({"role": "user", "content": prompt})
            st.session_state.messages.append(
                {"role": "assistant", "content": shutdown_message}
            )
            with st.chat_message("user"):
                st.markdown(prompt)
            with st.chat_message("assistant"):
                st.markdown(shutdown_message)
            shutdown_app()
            st.stop()
        st.session_state.messages.append({"role": "user", "content": prompt})
        with st.chat_message("user"):
